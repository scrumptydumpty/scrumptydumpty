...
const self = module.exports = {
  ...
  addTask: (title, description) => knex('tasks').insert({ title, description })
  ...
  getTasks: () => knex('tasks')
    .then((tasks) => {
      ...
      tasks.forEach((task) => {
        pchain = pchain.then(() => self.getBlockers(task.id)
          .then((blockers) => {
  ...
  updateTask: ({
    id, title, description, user_id, status_code, eta, priority_code, difficulty,
  }) => knex('tasks')
    .where('id', id)
    .update({
      title,
  ...
  getBlockers: task_id => knex('blockers').where('task_id', task_id)
  ...
  updateBlocker: ({
    ...
    .where('id', id)
    .update({
      title, description, task_id, status_code,
    })
    .then(() => knex('blockers')
      .where('id', id)
      .select())
  ...
  addUser: (username, password) => knex('users').insert({ username, password })
  ...
  userHasPassword: (username, password) => knex('users').where({ username, password }).select()
    .then(arr => arr.length !== 0),

  updateUser: (username, password) => knex('users').where('username', username).select()
    .then(arr => knex('users').where('id', arr[0].id).update({ username, password }))
    .then(() => knex('users').where('username', username).select())
    .then(users => users[0]),

  getUserByName: username => knex('users').where('username', username).select()
    .then(users => users[0]),

  getUserById: id => knex('users').where('id', id).select()
    .then(users => users[0]),
